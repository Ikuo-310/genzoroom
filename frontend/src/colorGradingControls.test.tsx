...
describe('Color Grading controls', () => {
  it('renders Shadows and Midtones Temperature/Tint below Color', () => {
    ...
    expect(Array.from(grading().querySelectorAll('label'), (item) => item.textContent)).toEqual(['Temperature', 'Tint', 'Temperature', 'Tint']);
    for (const control of [slider(), slider(1), slider(2), slider(3)]) {
  ...
  it('localizes the Shadows section label in Japanese', async () => {
    ...
    expect(Array.from(grading().querySelectorAll('label'), (item) => item.textContent)).toEqual(['色温度', '色かぶり補正', '色温度', '色かぶり補正']);
  ...
  it('bypasses and disables all four controls while preserving values across OFF/ON', () => {
    wheel(-1);
    wheel(1, false, slider(1));
    wheel(-1, false, slider(2));
    wheel(1, false, slider(3));
    ...
    expect([slider().value, slider(1).value, slider(2).value, slider(3).value]).toEqual(['10', '-10', '10', '-10']);
  });

  it('resets all four grading values as one operation and includes Midtones Tint in All Reset', () => {
    wheel(-1);
    wheel(-1, false, slider(1));
    wheel(-1, false, slider(2));
    wheel(-1, false, slider(3));
    ...
    expect(recipe()).toEqual(expect.objectContaining({ version: 14, colorGradingEnabled: true }));
