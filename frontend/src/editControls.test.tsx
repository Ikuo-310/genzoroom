...
describe('edit controls DOM interaction', () => {
  ...
  it('steps all adjustment types and commits the previous parameter when wheel input switches controls', () => {
    ...
    expect(session().recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0.1, contrast: 10, highlights: 10, whites: 10, shadows: 10, blacks: 10, shadowsTemperature: 0, vibrance: 0, saturation: 0 });
  ...
  it('uses one step for Shift+wheel across all adjustment types', () => {
    ...
    expect(session().recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0.01, contrast: 1, highlights: 1, whites: 1, shadows: 1, blacks: 1, shadowsTemperature: 0, vibrance: 0, saturation: 0 });
  });
  ...
  it('resets all Basic controls as one operation and restores them together with Undo', () => {
    ...
    expect(session().recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0, shadowsTemperature: 0, vibrance: 0, saturation: 0 });
...
describe('production Basic control wiring', () => {
  it.each([
    ...
    key('Enter', input);
    expect(session().recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0, shadowsTemperature: 0, vibrance: 0, saturation: 0, [adjustment]: Number(value) });
