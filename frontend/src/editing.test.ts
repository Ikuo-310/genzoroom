...
describe('non-destructive edit sessions', () => {
  it('starts with a serializable versioned zero recipe', () => {
    expect(JSON.parse(JSON.stringify(newSession().recipe))).toEqual({ version: 13, whiteBalanceEnabled: true, basicEnabled: true, colorGradingEnabled: true, colorEnabled: true, adjustments: { temperature: 0, tint: 0, exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 } });
  });
  ...
  it('resets all adjustments as one undoable All Reset operation', () => {
    ...
    expect(state.recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 });
    expect(state.history.at(-1)?.kind).toBe('allReset');
    const undone = editSession(state, { type: 'undo' });
    expect(undone.recipe.basicEnabled).toBe(false);
    expect(undone.recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0.35, contrast: 40, highlights: -55, whites: 45, shadows: 65, blacks: -70, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 });
    const redone = editSession(undone, { type: 'redo' });
    expect(redone.recipe.basicEnabled).toBe(true);
    expect(redone.recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 });
  });
  ...
  it('clamps tonal controls and keeps alternating adjustment history ordered', () => {
    ...
    expect(state.recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: -0.25, contrast: 100, highlights: -100, whites: -80, shadows: 100, blacks: -100, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 });
    ...
    expect(state.recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0.5, contrast: 100, highlights: -100, whites: -80, shadows: 100, blacks: -100, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 });
    state = editSession(state, { type: 'undo' });
    expect(state.recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0.5, contrast: 100, highlights: -100, whites: -80, shadows: 100, blacks: 0, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 });
    expect(editSession(state, { type: 'redo' }).recipe.adjustments).toEqual({ temperature: 0, tint: 0, exposure: 0.5, contrast: 100, highlights: -100, whites: -80, shadows: 100, blacks: -100, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0 });
...
describe('Basic scope', () => {
  ...
  it('preserves non-Basic values through reset, bypass, Undo and Redo', () => {
    const recipe = { ...newSession().recipe, adjustments: { temperature: 0, tint: 25, exposure: 0.5, contrast: 20, highlights: -30, whites: 40, shadows: 50, blacks: -60, shadowsTemperature: 0, shadowsTint: 0, midtonesTemperature: 0, vibrance: 0, saturation: 0, futureColor: 42 } };
