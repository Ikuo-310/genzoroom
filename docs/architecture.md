...
**Status: Early Development — authenticated photo browsing, Anshitsu, and minimal JPEG Exposure/Contrast/Highlights/Whites/Shadows adjustments are implemented. RAW development is not implemented.**
...
Temporary Immich preview adapter (editImageSource.ts)
  ...
  → exposurePipeline.ts (immutable source → linear-light Exposure gain 2^EV → sRGB midpoint Contrast → Highlights → Whites → Shadows)
  → GenzoRoom Canvas preview → existing Viewer transforms
```

The pipeline preserves alpha, clips display output to the sRGB range, and returns exact source bytes when all five adjustments are zero. Exposure converts sRGB to linear light, applies `2^EV`, and returns to sRGB. Contrast then scales each sRGB channel around the 0.5 midpoint by `1 + contrast / 100`. Highlights uses a smooth luminance weight above 0.5. Whites then uses smoothstep from luminance 0.75 to 1.0, interpolating luminance toward 1.0 for positive values or 0.75 for negative values. Shadows runs last with a smooth weight below 0.15, interpolating luminance toward `sqrt(Y)` for positive values or `Y²` for negative values. The tonal controls apply the resulting luminance ratio equally to RGB to limit hue shifts. Whites and Shadows protect division with a small epsilon, and every output channel is clipped to 0…1. Each update starts from the untouched decoded buffer, so clipping is not accumulated. Rendering is coalesced with requestAnimationFrame. No CSS filter, GPU, rendered asset, or backend change is involved. Source loads are aborted/ignored on asset changes and decoded bitmap resources are closed. Viewer source identity remains stable during adjustments, preserving Zoom/Pan.
...
- Further adjustments beyond JPEG Exposure, Contrast, Highlights, Whites, and Shadows: white balance, tone curve, HSL, and histogram tools, with waveform and RGB parade as later possibilities.
