...
History's header menu offers compact, clear, and reset; row context menus additionally offer partial deletion at the invoked row. Both use the same operation definitions and `organizeHistory()` handlers. Right-click or Shift+F10 opens a row menu without jumping. An explicit trim cursor deletes that row and earlier operations atomically: if the live cursor is newer, the live Recipe and pending work remain and the cursor decreases by the removed count; if older, the selected row's after Recipe becomes the starting point at cursor zero and replaces the pending preview. Organization Undo restores the entire original session including that pending work. Initial State's context menu uses the live cursor, with trim disabled at zero. Menus are portaled into a fixed viewport-clamped overlay and close on selection, Escape, Tab, outside pointer action, or photo navigation.

Clear and reset use a native `dialog.showModal()` confirmation with No initially focused, Tab/Shift+Tab cycling, Enter activation, Y/Yes, N/No, and Escape cancellation. IME, AltGraph, repeats, handled events, and modified Y/N input are rejected. The modal isolates background keyboard events, prevents other History and clipboard actions, and restores the invoking focus after it is re-enabled. Confirmation is bound to the original asset ID and rechecks edit availability at Yes. Compaction failures show an alert outside History; success adds no explanatory text. Persistence formats and save behavior are unchanged.
