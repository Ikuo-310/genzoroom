...
Temporary Immich preview adapter (editImageSource.ts)
  ...
  → exposurePipeline.ts (immutable source → linear-light Temperature → linear-light Tint → linear-light Exposure gain 2^EV → sRGB midpoint Contrast → Highlights → Whites → Shadows → Blacks → masked linear-light Shadows Temperature → Vibrance → Saturation)
  → GenzoRoom Canvas preview → existing Viewer transforms
```

effectiveAdjustments independently substitutes defaults for disabled White Balance, Basic, Color Grading, and Color values, preserving other fields. Category flags do not cause a pipeline-wide early return. The no-op check considers all eleven effective values and returns exact source bytes when they are zero. Alpha is preserved. Existing White Balance and tone formulas, luminance-region skips, intermediate rounding, and stage order are unchanged. After Blacks, Shadows Temperature calculates `Y = 0.2126R + 0.7152G + 0.0722B` from the current normalized sRGB bytes. With `x = clamp((Y - 0.20) / 0.30, 0, 1)`, its weight is `w = 1 - x²(3 - 2x)`: full strength through 0.20, a smooth fade across the lower midtones, and zero at and above 0.50. The existing Temperature gains are weighted in linear RGB as `effectiveGain = gain^w`, preserving reciprocal warm/cool behavior, then clipped, encoded back to sRGB, and rounded before Vibrance. Vibrance then uses the same luminance definition; its chroma is `max(|R-Y|, |G-Y|, |B-Y|)` and `lowSatWeight = 1 - clamp(chroma / 0.5, 0, 1)`. For positive values, `strength = 0.75 × lowSatWeight`; for negative values, `strength = 0.6 × (0.25 + 0.75 × lowSatWeight)`. It then uses `factor = 1 + vibrance / 100 × strength` and `outC = clamp(Y + (C-Y) × factor, 0, 1)`. The result is rounded to 8-bit before the unchanged Saturation stage. Zero-valued stages are skipped for byte identity. Each update starts from the untouched decoded buffer, so clipping is not accumulated. Rendering is coalesced with requestAnimationFrame. No CSS filter, GPU, rendered asset, or backend change is involved.
