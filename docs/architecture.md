...
**Status: Early Development — authenticated photo browsing, Anshitsu, and minimal JPEG Temperature/Tint/Exposure/Contrast/Highlights/Whites/Shadows/Blacks/Saturation adjustments are implemented. RAW development is not implemented.**
...
Temporary Immich preview adapter (editImageSource.ts)
  ...
  → exposurePipeline.ts (immutable source → linear-light Temperature → linear-light Tint → linear-light Exposure gain 2^EV → sRGB midpoint Contrast → Highlights → Whites → Shadows → Blacks → Saturation)
  → GenzoRoom Canvas preview → existing Viewer transforms
```

effectiveAdjustments independently substitutes defaults for disabled White Balance, Basic, and Color values, preserving other fields. Category flags do not cause a pipeline-wide early return. The no-op check considers all nine effective values and returns exact source bytes when they are zero. Alpha is preserved. Temperature decodes sRGB to linear RGB and applies gains R = 1.5^(-t/100), G = 1, B = 1.5^(t/100), with normalized integer t in −100…+100. At −100 the Red/Blue gains are 1.5 and 2/3; at +100 they are 2/3 and 1.5. Tint follows Temperature and applies R = 1.3^(u/100), G = 1.3^(-u/100), B = 1.3^(u/100), with normalized integer u in −100…+100. Each active White Balance stage clips to 0…1, encodes to sRGB, and rounds to bytes before the next stage. Existing tone formulas, luminance-region skips, intermediate rounding, and stage order are unchanged. After Blacks, Saturation uses the current 8-bit sRGB channels: `Y = 0.2126R + 0.7152G + 0.0722B`, `factor = 1 + saturation / 100`, and `outC = clamp(Y + (C - Y) × factor, 0, 1)` for each channel, then rounds back to 8-bit. Thus −100 is grayscale, 0 is skipped for byte identity, and +100 doubles chroma before clipping. Each update starts from the untouched decoded buffer, so clipping is not accumulated. Rendering is coalesced with requestAnimationFrame. No CSS filter, GPU, rendered asset, or backend change is involved. Source loads are aborted/ignored on asset changes and decoded bitmap resources are closed. Viewer source identity remains stable during adjustments, preserving Zoom/Pan.
...
- Further adjustments beyond JPEG Temperature, Tint, Exposure, Contrast, Highlights, Whites, Shadows, Blacks, and Saturation: tone curve, HSL, and histogram tools, with waveform and RGB parade as later possibilities.
