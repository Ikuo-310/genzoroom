...
**Status: Early Development — authenticated photo browsing, Anshitsu, and minimal JPEG Temperature/Exposure/Contrast/Highlights/Whites/Shadows/Blacks adjustments are implemented. RAW development is not implemented.**
...
Anshitsu owns an in-memory map keyed by Immich asset ID. Each session contains a JSON-serializable recipe (`{ version: 8, whiteBalanceEnabled: true, basicEnabled: true, adjustments: { temperature: 0, tint: 0, exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0 } }`), committed history entries (`kind`, `before`, `after`), a history cursor, and an optional pending transaction. Only JPEG assets are editable. Filmstrip switches preserve each asset's recipe and history; leaving Anshitsu or reloading discards them.

`editing.ts` provides pure begin/update/commit/reset/undo/redo transitions. Preview changes use the current recipe immediately. Pointer release/cancel, control unmount, or 500 ms keyboard inactivity commits one operation. Keyboard input resets the timer on every accepted keydown; keyup, pointer leave, and focus changes do not commit it early. No-op operations compare the complete recipe, including both category flags, Temperature, and Tint, and do not create history or discard redo. A new committed edit after Undo replaces the redo branch. Each adjustment Reset, White Balance/Basic category toggle, category Reset, and All Reset has a separate operation kind; all are undoable. Basic Reset restores the six parameter values in one operation without changing `basicEnabled` or White Balance values. White Balance Reset restores Temperature and Tint without changing `whiteBalanceEnabled`. All Reset restores all eight values and both category flags to true in one operation. Their History labels stay concise and are localized at display time rather than stored in recipes. The flat in-memory schema is version 8. No persistence or migration framework is introduced.

`AdjustmentSlider` supplies a compact reusable adjustment row with its label, native range input, synchronized direct numeric input, optional fixed-width unit slot, and inline reset in one responsive grid row. The label truncates when necessary while the range column consumes the remaining sidebar width. Direct input updates the current recipe only for finite normalized values, commits on Enter or blur, and restores its pre-edit value on Escape or invalid input. `editShortcuts.ts` protects text and number inputs, textarea, select, contenteditable, and IME input from the hover/focus slider shortcuts. Focused other sliders retain their own behavior. Exposure is bounded to −5…+5 EV in 0.01 EV increments with two displayed decimal places; Contrast, Highlights, Whites, Shadows, and Blacks are bounded to −100…+100 in integer steps. Temperature and Tint use −100…+100 integer steps, default 0, and no unit in a White Balance category above the six Basic rows. Negative Temperature warms and positive Temperature cools; negative Tint shifts toward Green and positive Tint toward Magenta. The optional trackGradient prop supplies a CSS custom property to opt-in Firefox/WebKit track styling for both White Balance rows, leaving Basic ranges unchanged. The future Color category below Basic is not implemented. Each category’s expanded state is local UI state that starts expanded on every Anshitsu entry and is not persisted. When either category is disabled, only its controls are disabled and dimmed while its values remain intact; only that category’s adjustments are bypassed. The temporary-preview notice remains outside the collapsible category.
...
Temporary Immich preview adapter (editImageSource.ts)
  ...
  → exposurePipeline.ts (immutable source → linear-light Temperature → linear-light Tint → linear-light Exposure gain 2^EV → sRGB midpoint Contrast → Highlights → Whites → Shadows → Blacks)
  → GenzoRoom Canvas preview → existing Viewer transforms
```

effectiveAdjustments independently substitutes defaults for disabled White Balance and Basic values, preserving other fields. Neither flag causes a pipeline-wide early return. The no-op check considers all eight effective values, including Temperature and Tint; it returns exact source bytes when they are zero. Alpha is preserved. Temperature decodes sRGB to linear RGB and applies gains R = 1.5^(-t/100), G = 1, B = 1.5^(t/100), with normalized integer t in −100…+100. At −100 the Red/Blue gains are 1.5 and 2/3; at +100 they are 2/3 and 1.5. Tint follows Temperature and applies R = 1.3^(u/100), G = 1.3^(-u/100), B = 1.3^(u/100), with normalized integer u in −100…+100. At −100 the R/G/B gains are 1/1.3, 1.3, 1/1.3; at +100 they are 1.3, 1/1.3, 1.3. Each active White Balance stage clips to 0…1, encodes to sRGB, and rounds to bytes before the next stage. At zero, that stage's round-trip is skipped entirely. Separate 256-entry channel LUTs avoid per-pixel exponentiation. Existing tone formulas, luminance-region skips, intermediate rounding, and stage order are unchanged. Exposure converts sRGB to linear light, applies `2^EV`, and returns to sRGB. Contrast then scales each sRGB channel around the 0.5 midpoint by `1 + contrast / 100`. Highlights uses a smooth luminance weight above 0.5. Whites then uses smoothstep from luminance 0.75 to 1.0, interpolating luminance toward 1.0 for positive values or 0.75 for negative values. Shadows uses a smooth weight below 0.15, interpolating luminance toward `sqrt(Y)` for positive values or `Y²` for negative values. Blacks runs last as a Master Black / Pedestal adjustment: `weight = 1 - smoothstep(0, 0.35, Y)` and `newY = clamp(Y + blacks / 100 × 0.10 × weight, 0, 1)`. The offset is strongest at black, remains visible through the 0.10–0.20 range, and fades smoothly to zero at 0.35. Non-black pixels use the resulting luminance ratio equally across RGB; exact black uses a neutral fallback because it has no defined hue and cannot be raised by multiplication. Shadows reshapes dark tonal separation, while Blacks shifts the low-range reference level. Every output channel is clipped to 0…1. Each update starts from the untouched decoded buffer, so clipping is not accumulated. Rendering is coalesced with requestAnimationFrame. No CSS filter, GPU, rendered asset, or backend change is involved. Source loads are aborted/ignored on asset changes and decoded bitmap resources are closed. Viewer source identity remains stable during adjustments, preserving Zoom/Pan.
...
- Further adjustments beyond JPEG Temperature, Tint, Exposure, Contrast, Highlights, Whites, Shadows, and Blacks: tone curve, HSL, and histogram tools, with waveform and RGB parade as later possibilities.
