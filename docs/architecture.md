...
**Status: Early Development — authenticated photo browsing, Anshitsu, and minimal JPEG Temperature/Tint/Exposure/Contrast/Highlights/Whites/Shadows/Blacks/Vibrance/Saturation adjustments are implemented. RAW development is not implemented.**
...
Temporary Immich preview adapter (editImageSource.ts)
  ...
  → exposurePipeline.ts (immutable source → linear-light Temperature → linear-light Tint → linear-light Exposure gain 2^EV → sRGB midpoint Contrast → Highlights → Whites → Shadows → Blacks → Vibrance → Saturation)
  → GenzoRoom Canvas preview → existing Viewer transforms
```

effectiveAdjustments independently substitutes defaults for disabled White Balance, Basic, and Color values, preserving other fields. Category flags do not cause a pipeline-wide early return. The no-op check considers all ten effective values and returns exact source bytes when they are zero. Alpha is preserved. Existing White Balance and tone formulas, luminance-region skips, intermediate rounding, and stage order are unchanged. After Blacks, Vibrance uses the current 8-bit sRGB channels and `Y = 0.2126R + 0.7152G + 0.0722B`. Its chroma is `max(|R-Y|, |G-Y|, |B-Y|)` and `lowSatWeight = 1 - clamp(chroma / 0.5, 0, 1)`. For positive values, `strength = 0.75 × lowSatWeight`; for negative values, `strength = 0.6 × (0.25 + 0.75 × lowSatWeight)`. It then uses `factor = 1 + vibrance / 100 × strength` and `outC = clamp(Y + (C-Y) × factor, 0, 1)`. This keeps positive Vibrance gentler than Saturation +100 and prevents Vibrance −100 from forcing colors fully gray. The result is rounded to 8-bit before the unchanged Saturation stage, which uniformly uses `factor = 1 + saturation / 100`. Zero-valued stages are skipped for byte identity. Each update starts from the untouched decoded buffer, so clipping is not accumulated. Rendering is coalesced with requestAnimationFrame. No CSS filter, GPU, rendered asset, or backend change is involved.
