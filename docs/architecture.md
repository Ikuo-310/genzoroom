...
**Status: Early Development — authenticated photo browsing, Anshitsu, and minimal JPEG Exposure/Contrast/Highlights/Whites/Shadows/Blacks adjustments are implemented. RAW development is not implemented.**
...
Anshitsu owns an in-memory map keyed by Immich asset ID. Each session contains a JSON-serializable recipe (`{ version: 6, basicEnabled: true, adjustments: { exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0 } }`), committed history entries (`kind`, `before`, `after`), a history cursor, and an optional pending transaction. Only JPEG assets are editable. Filmstrip switches preserve each asset's recipe and history; leaving Anshitsu or reloading discards them.

`editing.ts` provides pure begin/update/commit/reset/undo/redo transitions. Preview changes use the current recipe immediately. Pointer release/cancel, control unmount, or 500 ms keyboard inactivity commits one operation. Keyboard input resets the timer on every accepted keydown; keyup, pointer leave, and focus changes do not commit it early. No-op operations compare the complete recipe, including `basicEnabled`, and do not create history or discard redo. A new committed edit after Undo replaces the redo branch. Each adjustment Reset, Basic category toggle, Basic category Reset, and All Reset has a separate operation kind; all are undoable. Basic Reset restores the six parameter values in one operation without changing `basicEnabled`. All Reset restores the six values and `basicEnabled: true` in one operation. Their History labels stay concise and are localized at display time rather than stored in recipes. Adding `basicEnabled` changes the in-memory schema to version 6; a future persistent store must supply `basicEnabled: true` when migrating version 5 or earlier recipes, in addition to earlier field migrations. Persistent migration is still out of scope.

`AdjustmentSlider` supplies a compact reusable adjustment row with its label, native range input, synchronized direct numeric input, optional fixed-width unit slot, and inline reset in one responsive grid row. The label truncates when necessary while the range column consumes the remaining sidebar width. Direct input updates the current recipe only for finite normalized values, commits on Enter or blur, and restores its pre-edit value on Escape or invalid input. `editShortcuts.ts` protects text and number inputs, textarea, select, contenteditable, and IME input from the hover/focus slider shortcuts. Focused other sliders retain their own behavior. Exposure is bounded to −5…+5 EV in 0.01 EV increments with two displayed decimal places; Contrast, Highlights, Whites, Shadows, and Blacks are bounded to −100…+100 in integer steps. The six rows sit in the Basic category. Its expanded state is local UI state that starts expanded on every Anshitsu entry and is not persisted. When Basic is disabled, its controls are disabled and dimmed while its values remain intact; the preview pipeline returns the untouched source. The temporary-preview notice remains outside the collapsible category.
...
- Further adjustments beyond JPEG Exposure, Contrast, Highlights, Whites, Shadows, and Blacks: white balance, tone curve, HSL, and histogram tools, with waveform and RGB parade as later possibilities.
