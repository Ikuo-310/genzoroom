...
編集はAsset IDごとのメモリ上のsessionで保持する。必須adjustment追加に合わせてrecipeを `{ version: 5, adjustments: { exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0, blacks: 0 } }` とし、Historyは操作種別・変更前後recipe・cursorを持つ。Filmstrip切替では各Assetの状態を保持し、暗室を離れるかリロードすると消える。各個別ResetとAll Resetは別の操作種別で、いずれもUndo可能。All Resetは6項目を1操作で0へ戻す。将来永続化する際はversion 4 recipeへ `blacks: 0` を補うmigrationが必要になる。
...
検証：Frontend 147件のテストおよびproduction buildが成功。ローカルの640×360グラデーションfixtureを使うChromium確認では、Blacks 0 / +100 / −100の黒点側の変化、直接入力、10-stepキー操作、500ms commit、個別Reset、Undo/Redo、6項目のAll ResetとHistoryを確認した。右パネル230 / 約350 / 約440pxでは6本のslider端が揃い、パネル・ページとも横overflowは発生しなかった。230pxの保存値は再読み込みと折りたたみ後の再展開でも復元された。既存自動テストでViewer、Sidebar resize、Filmstrip、multi-select等の回帰を確認している。実Immich/NAS接続、実写真の色再現、大画像の性能、Firefoxは今回未検証。Backendは未変更で、Backendテストは今回再実行していない。
