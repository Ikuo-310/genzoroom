...
## JPEG Exposure / Contrast / Highlights / Whites / Shadows編集基盤（最新フェーズ）

JPEGのみを対象に、露光量 −5〜+5 EV（0.01 EV刻み、初期値0）、コントラスト・ハイライト・ホワイト・シャドウ −100〜+100（1刻み、初期値0）の調整を追加した。RAW/DNG・HEIC処理、Backend、DB、永続保存、Immich書き戻しには変更を加えていない。

編集はAsset IDごとのメモリ上のsessionで保持する。必須adjustment追加に合わせてrecipeを `{ version: 4, adjustments: { exposure: 0, contrast: 0, highlights: 0, whites: 0, shadows: 0 } }` とし、Historyは操作種別・変更前後recipe・cursorを持つ。Filmstrip切替では各Assetの状態を保持し、暗室を離れるかリロードすると消える。各個別ResetとAll Resetは別の操作種別で、いずれもUndo可能。All Resetは5項目を1操作で0へ戻す。
...
検証：Frontend 141件のテストおよびproduction buildが成功。ローカルの640×360 JPEGグラデーションfixtureを使うChromium確認では、Whites 0 / +100 / −100の明部変化と中間調・暗部維持、直接入力、10-stepキー操作、500ms commit、個別Reset、Undo/Redo、5項目のAll ResetとHistoryを確認した。右パネル230 / 350 / 440pxでは5本のslider端が揃い、横overflowは発生しなかった。既存自動テストでViewer、Sidebar resize、Filmstrip、multi-select等の回帰を確認している。実Immich/NAS接続、実写真の色再現、大画像の性能は今回未検証。Backendは未変更で、Backendテストは今回再実行していない。
